...
export default function App() {
  ...
  const [ThinkHmmFor2Sec, setThinkHmmFor2Sec] = useState(false);
  const [ThinkHmm, setThinkHmm] = useState(false);
  ...
  const handleLooksElement = (index) => {
    if (index===0) {
      ...
      setThinkHmmFor2Sec(true);
      setTimeout(() => {
        setThinkHmmFor2Sec(false);
      }, 2000);
    }
    else if(index===3){
      setThinkHmm(true);
          ...
          <div className="h-screen overflow-hidden flex flex-row ">
            ...
            <div className="w-1/3 h-full flex flex-row bg-white border-t border-l border-gray-200 rounded-tl-xl ml-2">
              <PreviewArea catPosition={catPosition} 
                            ...
                            ThinkHmmFor2Sec={ThinkHmmFor2Sec}
                            ThinkHmm = {ThinkHmm}
